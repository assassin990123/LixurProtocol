...
----------------------------------------------------------------------------------------------------------------------------------------------------------
*: Solve the public and private key errors for imported wallets (More than one day at most) [Done]
*: Solve the generating transaction thing with the imported addresses and invalid signatures. (A few days at most) [Done]
*:Redo the transaction adding process to have it resemble IOTA's transaction adding process [Done]
*: Add required parameters into the is_valid_transaction() function [Done]
*: We also have to update the cumulative weights of the transactions, via adding the logic to the update_cumulative_weight() function. (One to three days at most) [Done]
*: Solve the double asking problem for wallet signup (More than one day at most) [Done]
*: Might have to make people do Proof of Work on a transaction before it gets added to the graph. An option. (Not Urgent) [Done]
*: Add nonces during the Testnet Beta Version 1.0.0 or even before. Just look through your former code, analyze and append that code to it as well. (Not Urgent) [Done]
!: The refresh dilemma [Done]
*: Add the nonce in the transaction dictionary. [Done]
*: Add the mechanism where to get added to the chain, they must validate using the is_valid_transaction() function, two other transactions (the tips). Learn more online.[Done]
...
Notes: So I am pretty sure that I would have to add a mechanism where I would need to have a "waiting room" almost where transactions waiting to be validated.
